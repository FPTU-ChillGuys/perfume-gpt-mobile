...
const _statusTabs = <(String? value, String label, IconData icon)>[
  ...
  ('Approved', 'Đã duyệt', Icons.check_circle_outline_rounded),
  ('Rejected', 'Từ chối', Icons.cancel_outlined),
];

class CancelRequestListPage extends ConsumerStatefulWidget {
  const CancelRequestListPage({super.key});

  @override
  ConsumerState<CancelRequestListPage> createState() => _State();
}

class _State extends ConsumerState<CancelRequestListPage>
  ...
  Widget build(BuildContext context) {
    final dataAsync = ref.watch(
      myCancelRequestsProvider(status: _status, page: _page, pageSize: _pageSize),
    );
              ...
              child: ListView.builder(
                ...
                itemBuilder: (_, i) {
                  ...
                  return Padding(
                    ...
                    child: _CancelRequestCard(request: paginated.items[i]),
                  );
                },
              ),
            );
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.cancel_outlined, size: 48, color: AppColors.primary),
          ),
          const SizedBox(height: 20),
          const Text('Chưa có yêu cầu hủy đơn',
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
          const SizedBox(height: 6),
          const Text('Các yêu cầu hủy đơn sẽ hiển thị ở đây',
...
class _CancelRequestCard extends StatelessWidget {
  ...
  const _CancelRequestCard({required this.request});
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      elevation: 0,
      child: InkWell(
        onTap: () => context.push('/cancel-requests/${request.id}'),
        borderRadius: BorderRadius.circular(14),
        child: Container(
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(14),
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Refund amount
                    if (request.isRefundRequired && (request.refundAmount ?? 0) > 0) ...[
                      ...
                      Row(
                        children: [
                          ...
                          const SizedBox(width: 6),
                          Text(
                            _currencyFmt.format(request.refundAmount),
