# perfumegpt_ai_api_client.model.ProductControllerGetProductsBySemanticSearch200ResponseAllOfPayload

## Load the model package
```dart
import 'package:perfumegpt_ai_api_client/api.dart';
```

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**items** | [**List&lt;ProductWithVariantsResponse&gt;**](ProductWithVariantsResponse.md) |  | 
**pageNumber** | **num** | Số trang hiện tại | 
**pageSize** | **num** | Số bản ghi mỗi trang | 
**totalCount** | **num** | Tổng số bản ghi | 
**totalPages** | **num** | Tổng số trang | 

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


